    ...
    return (
        <div className="min-h-screen bg-gray-100 flex items-center justify-center">
            <div className="bg-white shadow-md rounded-lg p-8 w-full max-w-md">
                <button onClick={handleCreateSession} className="w-full bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600 transition-colors">
                    Create New Session
                </button>
                        ...
                        {sessionList.length > 0 ? (
                            <ul className="space-y-2">
                                {sessionList.map((session) => (
                                    <li key={session.id} className="py-2 px-4 border border-gray-200 rounded hover:bg-gray-50 transition-colors">
